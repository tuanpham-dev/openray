// Ported from src-tauri/src/application/system_commands.rs's `table()`.
// This build only ever runs on Linux (the native module compiled only its
// `#[cfg(target_os = "linux")]` branch here too), so only that branch is
// ported — the native file's macOS/Windows branches were dead code on this
// build target and are out of scope.
...
export const TABLE: SystemCommandMeta[] = [
  ...
  { id: 'show-desktop', title: 'Show Desktop', icon: 'monitor', keywords: ['minimize all', 'hide windows'], linuxRequires: ['wmctrl'] },
  { id: 'toggle-bluetooth', title: 'Toggle Bluetooth', icon: 'bluetooth', keywords: ['wireless'], linuxRequires: ['rfkill'] },
