...
/**
 * Import/Export hooks for Translate.
 *
...
/** Keys never worth carrying to another machine. Translation history is high-churn and machine-local, so it stays behind. `secret:` is
...
const EXCLUDED_PREFIXES = ['history:', 'secret:']
...
export async function importData(data: unknown, version: unknown): Promise<void> {
  if (version !== null && version !== undefined && version !== exportVersion) {
    throw new Error(`unsupported Translate export version ${String(version)} (this build understands ${exportVersion})`)
  }
